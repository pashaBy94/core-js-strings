...
function getStringLength(value) {
  if (value !== undefined && value !== null) return value.length;
  return 0;
}
...
function sumOfCodes(str) {
  return str && str.length > 0
    ? str.split('').reduce((ac, el) => ac + el.charCodeAt(0), 0)
    : 0;
}
...
function encodeToRot13(str) {
  const int = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  ...
  return str
    ...
    .map((word) =>
      word
        .split('')
        .map((el) => (int.indexOf(el) !== -1 ? out[int.indexOf(el)] : el))
        .join('')
    )
    .join(' ');
}
